...
# Load the dataset
data_path = '/Users/ahmed/Desktop/Machine Learning/cleaned_reviews.csv'
...
def preprocess_data(data, target_column):
    ...
    data = data.dropna(subset=[target_column, 'cleaned_review'])

    # Use cleaned_review_length as a feature and encode cleaned_review text as numerical data
    # Here we use the length of the text as a proxy for complexity
    data['text_length'] = data['cleaned_review'].apply(len)

    # Drop cleaned_review after deriving features
    ...
    categorical_cols = [col for col in data.select_dtypes(include=['object']).columns if col != target_column]
...
def minibatch_kmeans_coreset(X_train, y_train, fraction=0.05):
    ...
    # Select one sample from each cluster, handling empty clusters
    indices = []
    for i in range(coreset_size):
        ...
        if len(cluster_indices) > 0:
            ...
            # If a cluster is empty, select a random point from X_train
